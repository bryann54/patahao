...
import 'package:patahao/features/favourites/domain/usecases/add_to_favourites_usecase.dart';
import 'package:patahao/features/favourites/domain/usecases/check_if_fav_usecase.dart';
import 'package:patahao/features/favourites/domain/usecases/delete_favourite_usecase.dart';
import 'package:patahao/features/favourites/domain/usecases/load_favourites_usecase.dart';
...
class FavouritesBloc extends Bloc<FavouritesEvent, FavouritesState> {
  ...
  FutureOr<void> _listFavourites(
      ListFavouritesEvent event, Emitter<FavouritesState> emit) async {
  ...
  FutureOr<void> _addFavourite(
      AddFavouriteEvent event, Emitter<FavouritesState> emit) async {
    emit(LoadingFavourites(property: event.model));
    final response = await _addToFavouritesUsecase.call(event.model);
    emit(response.fold(
      (error) => AddFavouritesError(error: error.toString()),
      (favs) {
        add(CheckIfFavEvent(property: event.model));
  ...
  FutureOr<void> _deleteFavourite(
      DeleteFavouriteEvent event, Emitter<FavouritesState> emit) async {
    emit(LoadingFavourites(property: event.model));
  ...
  FutureOr<void> _checkIfFav(
      CheckIfFavEvent event, Emitter<FavouritesState> emit) async {
    emit(LoadingFavourites(property: event.property));
    final response = await _checkIfFavUsecase.call(event.property);
    emit(response.fold(
      (error) => CheckIfFavError(error: error.toString()),
      (isFav) =>
          CheckIfFavSuccess(property: event.property, isFav: isFav ?? false),
    ));
  }
}
